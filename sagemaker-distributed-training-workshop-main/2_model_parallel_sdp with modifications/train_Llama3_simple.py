...
# Import functions for translating state dictionaries
from smdistributed.modelparallel.torch.nn.huggingface.llama import (
    translate_hf_state_dict_to_smdistributed_llama,
    translate_state_dict_to_hf_llama,
)
...
# Set the logging level for the "torch.distributed.distributed_c10d" logger to "ERROR"
...
# Define a function for a single testing step
@smp.step
...
# Define a function for evaluating the model on a validation dataset
def eval_model(model, dataloader, num_batches):
    ...
    with torch.no_grad():
        ...
        for batch_idx, input_data in enumerate(dataloader):
            ...
            # Stop iterating if we've reached the desired number of batches
    ...
    # Compute the average loss and perplexity over all batches and all processes in a distributed training setup
    if n_batches > 0:
        torch.distributed.all_reduce(loss, group=smp.get_dp_process_group()) # sum the loss from all processes in the data parallelism group
        loss /= smp.dp_size()   # divide the total loss by the number of processes to get average loss
        loss /= n_batches       # divide the average loss by the number of batches to get average loss per batch
        loss = loss.item()      # convert the loss tensor to a Python floating-point number
        ppl = math.exp(loss)    # compute the perplexity as the exponential of the loss
    else:
        loss = -1.0            # if no batches were processed, set loss to -1.0
        ppl = -1.0

    return loss, ppl


def train(model, optimizer, lr_scheduler, model_config, start_train_path_index, start_batch_index, num_params, total_steps, args):# args is a dictionary that contains all the hyperparameters passed to the script
    if args.enable_memory_profiling > 0:
        memory_status_cpu(msg="before train step") # Memory_status_cpu is a function that prints the memory usage of the CPU
    model.train()           # Set the model to training mode
    
    train_paths = sorted( [os.path.join(args.training_dir, p) for p in os.listdir(args.training_dir)])# get the list of training paths

    train_dataloader = create_pretraining_dataloader( # create the training dataloader
        [train_paths[start_train_path_index]], # list of training paths
        args.train_batch_size,              # batch size
        args.max_context_width,             # max context width
        seed=args.seed,                     # random seed
        shuffle=True,                       # shuffle the data

    )

    if args.validation_freq is not None: # if validation frequency is not None
        # load all validation examples
        if smp.rank() == 0:
            print("Creating val dataloader") # print a message to the console
        val_paths = sorted([os.path.join(args.test_dir, p) for p in os.listdir(args.test_dir)]) # get the list of validation paths
        val_dataloader = create_pretraining_dataloader( # create the validation dataloader
            val_paths,
            args.val_batch_size,
        ...
        if smp.rank() == 0: # if the rank is 0
            print("Created val dataloader") # print a message to the console

    start = time.time() # get the current time
    throughput = None # initialize the throughput to None
    to_save = {"loss": [], "val_loss": []} # initialize the to_save dictionary with loss and validation loss
    loss_metric = 0

    def grad_accumulation_boundary(batch_idx): # define a function to check if the batch index is a gradient accumulation boundary
        return batch_idx % args.gradient_accumulation == args.gradient_accumulation - 1 # if the batch index is a gradient accumulation boundary
...
def parse_args(): # argparse arguments are passed through hyperparameters of PyTorch estimator
    parser = argparse.ArgumentParser()

    # hyperparameters sent by the client are passed as command-line arguments to the script.

    opt_grp = parser.add_argument_group(title="optimization", description="arguments for optimization")
    opt_grp.add_argument("--train_batch_size", type=int,default=4,
    ...
    opt_grp.add_argument("--activation_checkpointing",type=int,default=1,help="enable gradient checkpointing to reduce memory consumption",)
    parser.add_argument("--logging_freq", type=int, default=1, help="number of iterations between logging")
    ...
    io_grp.add_argument("--checkpoint-dir", type=str, default="/opt/ml/checkpoints", 
    ...
    io_grp.add_argument("--model-dir", type=str, default=os.environ["SM_MODEL_DIR"],
    ...
    io_grp.add_argument("--save_final_full_model", type=int, default=0, help="Enabling this will save a combined model only at the end",)
    ...
    model_grp = parser.add_argument_group(title="model", description="arguments to describe model configuration")
    ...
    parser.add_argument("--num_kept_checkpoints",type=int,default=5,help="how many checkpoints to keep before deleting",)
    parser.add_argument("--checkpoint_freq",type=int,default=10000,help="number of iterations between checkpointing",)
    parser.add_argument("--validation_freq",type=int,default=None,help="number of iterations to print validation loss",)
    parser.add_argument("--validation_batches",type=int,default=10,help="number of batches to estimate validation loss",)
    parser.add_argument("--use_fsx", type=int, default=0, help="Using FSx for checkpointing")
    parser.add_argument("--enable_memory_profiling", type=int, default=0, help="Enable memory profile")
    ...
    lr_grp.add_argument("--lr_decay_style",
                        type=str,default="linear", 
                        choices=["constant", "linear", "cosine", "exponential", "plateau"],
                        help="Learning rate decay function.", 
                        )
    lr_grp.add_argument("--lr_decay_iters", type=int, default=None,
                        help="number of iterations to decay learning rate over," " If None defaults to train iters",
                        )
    lr_grp.add_argument("--min_lr", type=float, default=0.0,
                        help="Minumum value for learning rate. The scheduler" "clip values below this threshold.",
                        )
    lr_grp.add_argument("--warmup", type=float,default=0.01,
        help="Percentage of total iterations to warmup on "
        "(.01 = 1 percent of all training iters).",
    )
    lr_grp.add_argument("--plateau", type=float,  default=0.4, help="Percentage of total iterations to keep at max if using plateau lr", )
...
def main():
    ...
    model_config = LlamaConfig(  # Changed to LlamaConfig for Llama-3
        vocab_size=args.vocab_size,
        n_positions=args.max_context_width,
        n_embd=args.hidden_width,
        n_layer=args.num_layers,
    ...
    if args.enable_memory_profiling > 0:                # if memory profiling is enabled
        memory_status_cpu(msg="before model creation")  # print the memory status before model creation

    if args.fp16 and args.bf16: # if both fp16 and bf16 are enabled
        raise ValueError("FP16 and BF16 cannot be simultaneously enabled.") # raise an error
    elif args.fp16:             # if fp16 is enabled
        dtype = torch.float16   # set the dtype to float16
    elif args.bf16:            # if bf16 is enabled
        dtype = torch.bfloat16  # set the dtype to bfloat16
    else:
        dtype = torch.get_default_dtype() # if neither fp16 nor bf16 are enabled, set the dtype to the default dtype

    with smp.model_creation( # create the model with the specified configuration
        tensor_parallelism=smp.tp_size() > 1 or args.use_distributed_transformer > 0, # if tensor parallelism is enabled
        dtype=dtype, # set the dtype to the specified dtype
        attention_in_fp32=args.attention_in_fp32 > 0, # if attention is in fp32 
        query_key_layer_scaling=args.query_key_layer_scaling > 0 and args.bf16 < 1, # if query key layer scaling is enabled
        fused_softmax=args.fused_softmax > 0, # if fused softmax is enabled
        fused_bias_gelu=args.fused_bias_gelu > 0, # if fused bias gelu is enabled
        ):
            model = AutoModelForCausalLM.from_config(model_config) # create the model from the specified configuration
    if args.enable_memory_profiling > 0:
    ...
    optimizer = optim.AdamW(param_groups, betas=(args.beta1, args.beta2), lr=args.lr, weight_decay=args.weight_decay)
    ...
    optimizer = smp.DistributedOptimizer(optimizer, static_loss_scale=None, dynamic_loss_scale=True, dynamic_loss_args={"scale_window": 1000, "min_scale": 1, "delayed_shift": 2},
                                         )
    lr_scheduler = get_learning_rate_scheduler(optimizer, args)

    if args.enable_memory_profiling > 0: # if memory profiling is enabled
        model.register_post_partition_hook( # register a post partition hook
            lambda model, optimizer: memory_status(msg="After_partition") # print the memory status after partitioning
        )

    # load after wrapping model and optimizer with smp Distributed...
    if args.load_full or args.load_partial: # if loading from a full or partial checkpoint
        if args.load_partial and args.load_full: # if both partial and full checkpoints are set
            print("Since both --load_partial and --load_full set, will try to load from full checkpoint." 
                "If the intention is to load from partial checkpoint, please don't set --load_full"
            )
        partial = not args.load_full # if loading from a full checkpoint, set partial to False, otherwise set it to True    
        path = args.checkpoint_dir if partial else args.model_dir # set the path to the checkpoint directory if loading from a partial checkpoint, otherwise set it to the model directory
        tag = None if partial else "fullmodel.pt" # set the tag to None if loading from a partial checkpoint, otherwise set it to "fullmodel.pt"
        user_content = smp.resume_from_checkpoint(path, tag=tag, partial=partial) # resume from the checkpoint
        total_steps = user_content["total_steps"] if partial else 0 # set the total steps to the total steps in the checkpoint if loading from a partial checkpoint, otherwise set it to 0
        start_train_path_index = user_content.get("start_train_path_index", 0) # get the start train path index from the checkpoint if it exists, otherwise set it to 0
        start_batch_index = user_content.get("start_batch_index", 0) # get the start batch index from the checkpoint if it exists, otherwise set it to 0
        if "lr_scheduler" in user_content: # if the lr scheduler is in the checkpoint
            lr_scheduler.load_state_dict(user_content["lr_scheduler"]) # load the lr scheduler state from the checkpoint
    else:
        total_steps = 0
    ...
    start = time.time()
    total_steps, throughput, loss = train( # train the model
        model,                  # the model
        optimizer,              # the optimizer
        lr_scheduler,           # the learning rate scheduler
        model_config,           # the model configuration
        start_train_path_index, # the start train path index
